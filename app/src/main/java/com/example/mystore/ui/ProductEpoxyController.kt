...
class ProductEpoxyController:TypedEpoxyController<List<UiProduct>>() {
    ...
    override fun buildModels(data: List<UiProduct>?) {
        if (data.isNullOrEmpty()){
            repeat(7) {
                val epoxyId = it + 1
                ProductEpoxyModel(combinedProduct = null).id(epoxyId).addTo(this)
            }
            return
        }
    ...
    data class ProductEpoxyModel(val combinedProduct: UiProduct?)
        ...
        override fun ModelProductItemBinding.bind() {
            ...
            combinedProduct?.let { combinedProduct->
                ...
                val imageRes=if (combinedProduct.isFavorite) R.drawable.ic_round_favorite_24 else R.drawable.ic_round_favorite_border_24
